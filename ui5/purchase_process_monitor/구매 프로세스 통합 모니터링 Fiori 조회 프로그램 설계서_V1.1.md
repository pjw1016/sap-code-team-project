# 구매 프로세스 통합 모니터링 Fiori 조회 프로그램 설계서 V1.1
...
본 문서는 기존 V1.0 설계서와 추가 인터뷰에서 확정한 업무 규칙을 하나로 통합한 개발 기준 문서다.
...
5. RFQ 견적 접수 판정 방식
6. OData EntitySet 및 필드 계약
7. Backend와 UI5 Frontend의 책임 분리
8. 전체 개발 순서
```
...
| 구분 | V1.0 | V1.1 확정안 |
| --- | --- | --- |
| Begin 목록 단위 | 문서 또는 품목 단위가 혼재 | 문서 Header 단위 |
| 지연 계산 단위 | 명확하지 않음 | 문서 Item 단위 |
| KPI 집계 단위 | 건수 기준만 정의 | 지연된 고유 Header 문서 수 |
| Header 지연 판정 | 별도 규칙 없음 | Item 중 하나라도 지연이면 Header 지연 |
| Header 대표 상태 | 별도 규칙 없음 | 가장 후속 단계의 지연상태 |
| Header 지연일수 | 별도 규칙 없음 | 대표 상태 Item 중 최대 지연 Working Day |
| RFQ 지연 명칭 | RFQ 미응답 | RFQ 견적 미접수 |
| RFQ 완료 기준 | 공급업체별 응답 전제 | 참조 MQ Item이 1건 이상이면 접수 완료 |
| 공급업체 현황 | 공급업체별 응답 현황 | RFQ별 견적 접수 현황 |
| 지연일 계산 | 달력일 상수 기준 | Factory Calendar Working Day 기준 |
| 조회기간 | 고정 범위 | 최근 3개월/6개월/12개월 Select |
| 오래된 미완료 문서 | 별도 규칙 없음 | 조회기간과 관계없이 포함 |
| 긴급발주 건수 | 금주 요약에 포함 | 판정 필드가 없어 V1.1에서 제외 |
| Mid 최초 표시 | 관련 문서 전체 | 전체 품목 표시, 지연 품목 우선 정렬 |
...
| RFQ 생성 후 견적 미접수 | RFQ 견적 미접수 KPI 및 RFQ별 견적 접수 현황 |
| MQ 접수 후 거래선 미채택 | 견적 채택 지연 KPI |
...
개발 마감 일정을 고려하되, V1.1은 설계된 전체 기능을 구현 범위로 한다.
...
### 5.7 RFQ 상태의 Item 환산 규칙

RFQ 견적 접수 여부는 Header 단위로 판정한다.

```text
RFQ Header에 연결된 MQ가 1건 이상 존재
→ RFQ Header 접수 완료
→ 해당 RFQ의 모든 Item은 RFQ 견적 미접수 대상에서 제외

RFQ Header에 연결된 MQ가 0건
AND 허용기한 초과
→ RFQ Header 견적 미접수
→ 해당 RFQ의 전체 Item을 지연 Item으로 집계
```

따라서 `RfqNoQuoteItemCount`는 공급업체별 미응답 수가 아니라 견적 미접수 RFQ Header에 포함된 Item 수다.
...
### 7.2 Factory Calendar

Backend에서는 프로젝트에서 사용 중인 `BKK*` 계열 Function Module을 활용한다.

개발 전에 SE37 및 SCAL에서 다음 항목을 반드시 확인한다.

| 확인 항목 | 확인 내용 |
| --- | --- |
| Function Module | 실제 사용 가능한 정확한 명칭 |
| Calendar ID | 프로젝트에서 사용하는 한국 기준 Factory Calendar |
| Holiday Calendar | 대한민국 공휴일 설정 여부 |
| 시작일 포함 | 기준일을 Working Day 계산에 포함하는지 |
| 종료일 포함 | 조회 기준일을 포함하는지 |
| 휴일 입력 | 시작일 또는 종료일이 휴일인 경우 처리 |
| 반환값 | Working Day 수 또는 계산된 날짜 반환 방식 |

### 7.3 계산 규칙

```text
허용기한 = 기준일자에 허용 Working Day를 더한 날짜

조회 기준일 > 허용기한
→ 지연

지연일수
= 허용기한 다음 Working Day부터 조회 기준일까지의 Working Day 수
```
...
| 기준일자 | RFQ 생성일 |
| 허용일수 | 2 Working Day |
| 접수 완료 | 해당 RFQ Item을 참조하는 MQ Item이 1건 이상 존재 |
...
MQ가 1건 이상 등록되면 공급업체 수와 관계없이 RFQ 견적 접수 완료로 본다.
...
| --- | --- |
| 기준문서 | RFQ/RQ |
| 기준일자 | 해당 RFQ를 참조하는 MQ 중 마지막 MQ 생성일 |
| 허용일수 | 1 Working Day |
...
```text
참조 MQ Item 수 > 0
AND 마지막 MQ 생성일 기준 허용기한 < 조회 기준일
AND 채택 MQ Item 없음
→ 견적 채택 지연
```

RFQ별 별도 견적 마감일이 없으므로 마지막 MQ 생성일을 채택 검토 시작 기준으로 사용한다.
...
| 완료 조건 | 정상 IV가 존재하고 송장수량 또는 금액이 기준을 충족 |
...
실제 `RBSTAT`, `IVSTAT` 값과 취소 처리 필드는 Backend 개발 전에 테이블 정의서에서 검증한다.
...
긴급발주 건수는 V1.1에서 제외한다.

### 11.4 RFQ별 견적 접수 현황

V1.0의 공급업체별 응답 현황을 RFQ별 견적 접수 현황으로 변경한다.
...
접수상태는 2가지로 표시하되, 지연 여부는 별도 필드로 구분한다.
...
Item이 여러 지연상태에 해당하면 대표 Item 상태와 추가 상태를 함께 반환한다.
...
| `RfqNoQuoteItemCount` | RFQ 견적 미접수 Item 수 |
...
| `ReceptionStatus` | `NOT_RECEIVED` 또는 `COMPLETED` |
| `ReceptionStatusText` | 미접수 또는 완료 |
| `DelayYn` | 지연 여부 |
| `Criticality` | UI 상태 |
...
| RFQ 참조 MQ 1건 | RFQ 접수 완료 |
| MQ 존재, SELIDC 없음 | 허용일수 초과 시 견적 채택 지연 |
| MQ 중 SELIDC 1건 | 견적 채택 완료 |
...
| RfqQuotationStatusSet | MQ 0/1건 상태 |
...
| 8 | 프로젝트에서 사용할 정확한 `BKK*` Function Module |
| 9 | Factory Calendar ID와 한국 공휴일 설정 |
...
├─ package.json
├─ ui5.yaml
└─ 구매 프로세스 통합 모니터링 Fiori 조회 프로그램 설계서_V1.1.md
```
...
- PR~IV 연결 Key 확인
- GR/IV 상태값 확인
- BKK Function Module과 Factory Calendar 확인
- 기존 앱 Intent 정보 확인
```
...
| 6 | Item 지연 계산 | Backend | 5개 상태 판정 성공 |
| 7 | Header 집계 | Backend | 대표 상태/KPI 기준 일치 |
...
| 13 | Begin 전체 화면 | Frontend | KPI/요약/Header 목록 완료 |
| 14 | ProcessFlow | Frontend | 단계별 집계 표시 |
| 15 | 품목/관련 문서 | Frontend | 지연 Item 식별 가능 |
